...
static const Case CASES[] = {
    ...
    { .name = "select-bg-content-width",
      .args = { "select", "--bg", "blue", "--width", "content",
                "--min-width", "20", "a", "b" },
      .keys = "\x1b[B\r", .want_stdout = "b\n", .want_exit = 0 },
    /* default cycles: Up on the first row jumps to the last */
    { .name = "select-cycle-default",
      .args = { "select", "a", "b", "c" },
      .keys = "\x1b[A\r", .want_stdout = "c\n", .want_exit = 0 },
    /* --no-cycle: Up on the first row stays on the first */
    { .name = "select-no-cycle",
      .args = { "select", "--no-cycle", "a", "b", "c" },
