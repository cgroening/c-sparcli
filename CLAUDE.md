...
- Word-wrap in tables (`ScColOpts.wrap = 1`) breaks on spaces only. If no space fits in the column width, the line is truncated. **Truncated (non-wrapped) cells end with an ellipsis (`…`)** — `render_cell_line_truncated` reserves one column for the glyph and trims the content to fit (`remaining - 1` columns + `…`); a column only 1 cell wide shows just `…`. The ellipsis inherits the cell's style/bg.
