...
**Runtime overrides** – `sc_palette_set(name, color)` / `sc_palette_get(name, &out)` / `sc_palette_reset()` (`core/sparcli_core.h`, impl `src/core/color.c`) recolor any name at runtime. Backed by a mutable array parallel to the static `sc_named_colors[]` (zero-init slot = no override; `index==0` color clears one); `sc_color_by_name_n` returns the override when present, so the change flows to **markup/CLI/args** *and* to widget defaults that resolve a palette name – notably the **fuzzy** finder's accent default now resolves `"accent"` (so `sc_palette_set("accent", …)` retints the selected row + match highlight; default tone is the palette teal, not ANSI cyan). This is the *name→color* palette, distinct from a widget's `accent` field: to retint the input widgets directly, use `sc_input_set_theme(&(ScInputTheme){ .accent = … })`. Set-once before spawning threads (theme/logger pattern); resolution is read-only. Tests: `tests/input/logic/test_palette.c`.

**Bindings:** C++ `sparcli::palette::accent()` (functions, since the C macros are compound literals); Rust `sparcli::palette::ACCENT` (`pub const Color`); Python `sc.Palette.ACCENT` (RGB `Color`). The bindings expose all 53 colors (including the soft hues, namespaced so they don't shadow the ANSI ones). The runtime overrides are mirrored too: C++ `sparcli::palette::set/get/reset`, Rust `sparcli::palette::set/get/reset` (`Color` is now `PartialEq`), Python `sc.Palette.set/get/reset`. The canonical hex list lives in `include/core/sparcli_palette.h`; all language copies are generated from it – keep them in sync when editing.
...
typedef struct {
    ...
    bool     no_stretch; /* size to content; exclude from total_width stretching */
} ScColOpts;
```

**`total_width` stretching (`no_stretch`):** `ScTableOpts.total_width` distributes the surplus/deficit across **flex** columns. A column is flex when `fixed_width == 0` **and** `no_stretch == false`; setting `no_stretch` keeps the column at its content width (still honoring `min/max`) and routes the extra space to the remaining flex columns. The fuzzy table view uses this for `ScFuzzyOpts.stretch_columns`.
...
`sc_key_decode(buf, len, &key)` is a **pure, unit-tested** decoder (`ScKey { type, codepoint, bytes }`) handling control bytes, CSI/SS3 sequences (arrows, Home/End, Delete, PageUp/Down, Shift+PageUp/Down via the `;2` modifier, Shift-Tab) and multi-byte UTF-8. **Modified named keys** carry the xterm `ESC[1;<mod>X` parameter: the key type is unchanged (arrow/Home/End) and the modifier bits are set on `ScKey.mods` (`SC_MOD_SHIFT`/`SC_MOD_ALT`/`SC_MOD_CTRL`; `mod = code+1`, bit0 Shift, bit1 Alt, bit2 Ctrl – so `ESC[1;3A` = Alt+Up). A bare `ESC[1A`/`ESC[1H` is the plain key; a real numeric parameter (`ESC[5A`) is still `SC_KEY_NONE`. It returns 0 bytes consumed for incomplete prefixes (lone ESC, partial UTF-8/CSI); the buffered reader `sc_tty_read_key` resolves a lone ESC to `SC_KEY_ESC` via a 25 ms `select()` timeout (not `poll()` – `poll()` on `/dev/tty` is broken on macOS and would swallow a lone Esc until the next key). Unrecognized escape sequences (e.g. pasted ANSI color codes) decode to `SC_KEY_NONE` with all bytes consumed; `sc_tty_read_key` **skips them silently** – they neither cancel the prompt nor appear as text, and `SC_KEY_NONE` from the reader always means EOF/read error. The decoder also recognizes the bracketed-paste markers (`ESC[200~`/`ESC[201~` → `SC_KEY_PASTE_START`/`_END`); between them the reader switches to a literal-text decode (`decode_paste_content`) that flags every key with `SC_MOD_PASTED`.
...
void      sc_fuzzy_add_section(ScFuzzy *f, const char *title); /* non-selectable header */
...
void      sc_fuzzy_add_row_rich(ScFuzzy *f, ScText *const *cells, size_t n); /* full ScText cells; match-key flattened from spans */
void      sc_fuzzy_set_disabled(ScFuzzy *f, size_t i, bool on); /* greyed, non-selectable */
void      sc_fuzzy_set_id(ScFuzzy *f, size_t i, uint64_t id); uint64_t sc_fuzzy_id_at(const ScFuzzy *f, size_t i); uint64_t sc_fuzzy_cursor_id(const ScFuzzy *f);
ScInputStatus sc_fuzzy_run(ScFuzzy *f, size_t *out_index);  /* out_index = original add order */
ScInputStatus sc_fuzzy_run_multi(ScFuzzy *f, size_t *indices, size_t *count_io); /* opts.multi; checked add-order indices */
void      sc_fuzzy_set_checked(ScFuzzy *f, size_t i, bool on); bool sc_fuzzy_is_checked(const ScFuzzy *f, size_t i); void sc_fuzzy_check_all(ScFuzzy *f, bool on); size_t sc_fuzzy_checked_count(const ScFuzzy *f);
void      sc_fuzzy_set_cursor(ScFuzzy *f, size_t i); const char *sc_fuzzy_label(const ScFuzzy *f, size_t i); void sc_fuzzy_set_label(ScFuzzy *f, size_t i, const char *s); void sc_fuzzy_set_row(ScFuzzy *f, size_t i, const char *const *fields, size_t n); void sc_fuzzy_set_row_style(ScFuzzy *f, size_t i, size_t col, ScTextStyle s);
void      sc_fuzzy_free(ScFuzzy *f);
bool      sc_fuzzy_match(const char *pattern, const char *str, int *score);  /* pure, testable */

/* Form: grid-layout of framed fields, 2D-navigated, edited below the grid */
...
void      sc_form_add_skip(ScForm *f);                                        /* placeholder under a col/row span */
ScInputStatus sc_form_run(ScForm *f);                                         /* Ctrl-D submit (enforces required), Esc cancel */
const char *sc_form_get_string(const ScForm *f, int field); double sc_form_get_number(const ScForm *f, int field); bool sc_form_get_bool(const ScForm *f, int field);
...
bool      sc_form_get_date(const ScForm *f, int field, struct tm *out);       /* date field */
...
- **Fuzzy** ranks by `sc_fuzzy_match` on each keystroke; matched characters are highlighted (bold+underline, accent fg) in the list and in the matched table cells (`render_table` builds those cells as highlighted `ScText` via `append_highlighted`, borrowed by the table and freed after capture); the query field scrolls horizontally (`‹`/`›`) when long; table view builds an `ScTableData` of the visible rows each frame (cursor row via row-bg) and `sc_vstack`s query + body + scroll-indicator + footer. `refilter` matches each row via `row_matches` across the columns selected by `opts.search_columns` (bitmask; `0` = all, the default), ranking by the best-scoring column – so a table query can hit (and highlight) any column, not just the first. The cursor cycles around the result-list ends by default; `opts.no_cycle` disables it. **Section navigation:** when sections are present (`sc_fuzzy_add_section`), **Tab jumps to the next section** (its first selectable row) and **Shift-Tab to the previous section's first row**, both cycling around the ends (`section_jump` over the `matches[]` display list); without sections Tab/Shift-Tab fall back to cursor down/up. Arrows / `j`/`k` / Ctrl-N/P stay single-step. **Scroll anchoring:** `scroll_to_cursor` anchors the viewport to a section header that sits directly above the cursor (the cursor never lands on a header), so landing on a group's first row scrolls its title into view and the scrollbar/▲ indicator sit at the very top (not one row below the hidden header). **`opts.max_height`** (rows): caps the finder height so the result list scrolls within it (the existing `top`+`effective_visible` viewport) instead of overflowing; `0` auto-fits the terminal height, or set it to a live dashboard's `prompt_rows` reserve. `effective_visible()` is **view-aware**: it subtracts the chrome around the result body (query line, scroll indicator, box frame, own hint footer, **and the engine's labeled-shortcut footer** via `sc_shortcut_hint_rows` – which returns the footer's **soft-wrapped** height `ceil((indent + visible_width) / term_width)`, so a long footer that wraps to several terminal rows reserves them all instead of being clipped), then converts the remaining screen rows to *entries* – 1 row per entry in the list view, but in the **table view** it divides by the table's real per-row cost (borders, header row + separator, inter-row separators via `fuzzy_table_overhead`, assuming single-line rows) so a bordered/separated table never overflows the budget. `sc_fullscreen_compose` takes a `bottom_reserve` (the shortcut-footer rows) so that footer, stacked by the engine after `render`, stays on-screen. When the list scrolls, a **right-edge scrollbar** (track `░` / thumb `█`, `attach_scrollbar` appends a gap + bar column to the body, reserving 2 interior cols) is drawn **by default** alongside the `↑ x–y/total ↓` text line; `opts.no_scrollbar` opts out. **`sc_fuzzy_refresh()`**: re-applies the query after appending rows mid-run (the `if (matches) refilter()` path, which now grows the `matches` array to the new row count); bind it from a `SC_SHORTCUT_CALLBACK` "refresh" key to add items live without closing the finder. **`opts.stretch_columns`** (bitmask, table view): when the `box` resolves to a width that **differs from** the natural table (`SC_WIDTH_FULL`/`FIXED`, or `CONTENT`+`max_width`), the table is re-rendered with `total_width = interior` and the selected columns absorb the difference (the rest get `ScColOpts.no_stretch`); `0` = content-sized as before. Wider interior → the slack goes to the stretch columns; **narrower interior** (e.g. an over-long cell/title) → the stretch columns shrink and their cells truncate with `…`, so the fuzzy table never overflows the terminal. The list view already fills a bounded box, so this is table-only. **`opts.fullscreen`** (+ `opts.header` borrowed `ScRendered`, `opts.valign`): full-screen mode – the finder fills the terminal, its list **grows with the items** up to the available height (`sc_terminal_height() − header_lines − chrome`) and only then scrolls, and each frame `fuzzy_render` wraps the result with `sc_fullscreen_compose(frame, header, NULL, valign, SC_VALIGN_SCOPE_ALL, footer_reserve)` (the finder uses the whole-block `ALL` scope and no pinned footer) so the pinned header + the leftover-space alignment (below the finder for TOP, above the header for BOTTOM, both for MIDDLE) **re-compute live** as the list grows. The finder does **not** own the alternate screen – run it inside an `sc_altscreen_begin()`/`sc_altscreen_end()` session (so switching widgets don't flicker).
  - **Rows are an array-of-structs** (`Row { fields, styles, texts, ncols, id, is_section, disabled, checked }`, `src/input/fuzzy.c`); the filtered view is the `matches[]` **display list** of `{index, score, is_section}` entries, interleaving section headers with their rows. The cursor only lands on **selectable** entries (`!is_section && !disabled`); `prev/next/first/last_selectable` skip the rest, `clamp_cursor_selectable` re-snaps after a refilter.
  - **Sections** (`sc_fuzzy_add_section`): non-selectable headers. `refilter` emits each group as `[header][its matches]` and the header **only when the group matched** (`opts.section_counts` appends `" (k)"`). In the table they render as a colspan row over all columns. `opts.empty_text` shows a line when nothing matches. **Per-section styling:** `sc_fuzzy_add_section_styled(title, ScTextStyle)` gives a header its own bg/fg/attr (`.bg` fills the full-width bar), and `sc_fuzzy_add_section_text(ScText *title, ScTextStyle fill)` takes a rich multi-span title (`fill.bg` paints the bar + styles the count). Both reuse the row's existing `styles[0]` / `texts[0]` slots (no new `Row` fields); `section_style_for` merges the row's `styles[0]` over the global `section_style` (set fields win), and both render paths append the rich `texts[0]` when present.
  - **Ordering** (`opts.order`: `SCORE`/`INSERTION`/`COLUMN` + `order_column`/`order_desc`) is applied **per group** via the `match_cmp` comparator reading a static `g_sort_ctx` around the `qsort` calls (single-session, like the rest of the input layer).
  - **Per-cell color**: `add_row_styled` carries a per-cell base `ScTextStyle` (the match highlight overlays it; on the cursor row `merge_cursor_style` **unions** the attributes – the cell keeps its own attrs (e.g. `dim`+`strike` on a done task) and the highlight's bold is added on top, not replacing them – while the cursor fg/bg override); `add_row_rich` takes full `ScText` cells (multi-color spans, no char highlight) and flattens their span text via `sc_text_span` for the match/label key. Disabled rows render in `disabled_style` (dim).
  - **Multi-select** (`opts.multi`): Space toggles `Row.checked`, `sc_fuzzy_run_multi` returns the checked add-order indices; `set_checked`/`is_checked`/`check_all`/`checked_count` seed/inspect it; `opts.checkbox_column` gives the table a dedicated leading checkbox column (else the glyph prefixes column 0); `opts.toggle_all_key`/`toggle_section_key` (`ScKeyChord`, matched in `on_key`) flip all/the current group. Disabling a row clears its check (it leaves the selection).
  - **Stable ids + live mutation**: `set_id`/`id_at`/`cursor_id` give rows caller ids robust against `remove`/reorder; `set_cursor`/`opts.initial_query` seed the run; `set_label`/`set_row`/`set_row_style`/`remove` mutate live (re-`refilter` while a run is active) – the basis for the apply-action-then-re-run loop in `examples/c/apps/todo_fuzzy.c`. Tests: `tests/input/logic/test_fuzzy.c` (`test_fuzzy_logic`), PTY cases `fuzzy-section-skip`/`-disabled-skip`/`-multi-toggle`/`-toggle-all`/`-toggle-section`/`-order-column`/`-initial-query`/`-multi-shortcut`, style snapshots in `test_style_fuzzy.c`. Bindings: C++ `sparcli::Fuzzy`, Rust `sparcli::Fuzzy`/`FuzzyOrder`, Python `sc.Fuzzy`/`sc.FuzzyOrder`.
  - **Modal normal/insert mode** (`opts.modal`, off by default): a vim-style split. In **normal** mode bare-letter shortcuts fire (the engine matches them via the new `suppress_char_shortcuts` predicate, false in normal mode), `j`/`k`/`g`/`G` navigate, the configured `clear_key` (zero-init = disabled) empties the query, and `Esc` cancels. In **insert** mode keys edit the query and bare-letter shortcuts **plus the editing keys (Backspace/Delete)** are suppressed (so they edit the query instead of firing a shortcut bound to them); they still fire in normal mode. Toggle with `insert_key` (zero-init = `i`) and `normal_key` (zero-init = `Esc`); leaving insert via `Esc` works because the engine's new `capture_escape` vtable flag forwards `Esc` to `on_key` instead of cancelling (`Ctrl-C`/EOF still cancel). `start_in_insert` picks the initial mode (default normal). The query line shows the active mode: a colored field tint + a `NORMAL`/`INSERT` badge (labels `normal_label`/`insert_label`, styles `mode_normal_style`/`mode_insert_style`; `hide_mode_badge` drops the badge). `Ctrl-N`/`Ctrl-P` move the cursor in **both** modes (and now for the non-modal finder too – the previously documented-but-missing behavior). PTY cases `modal-*` + `fuzzy-ctrl-n-nav`, style snapshots `modal NORMAL/INSERT badge`. Bindings expose the opts (Rust `FuzzyOpts::modal()`/`.start_in_insert()`/`.mode_labels`/`.mode_styles`/`.mode_keys`/`.clear_key`, Python kwargs, C++ via `FuzzyOpts`) plus a bare-char chord builder `key_char` (C++/Rust/Python) for `clear_key` and bare-letter shortcuts.
- **DatePicker** renders a month grid; arrows move day/week, PageUp/Down or `<`/`>` change month, Shift+PageUp/Down change year; zeroed `struct tm` seeds today. Month/year jumps keep the selected day, clamped to the target month's last valid day (e.g. Jan 31 → Feb 28). `week_start` is `ScWeekStart`. `ScDatePickerOpts.allow_clear` lets Delete/Backspace return "no date": `sc_datepicker` then returns `SC_INPUT_OK` with a zeroed `*io` (the `sc_date_is_empty()` sentinel – year/mon/mday all 0), so an optional/absent date is representable; the footer gains a "del clear" hint.
...
- **Form** (`form.c` + `include/input/sparcli_form.h`): grid-layout form. Fields are framed boxes (label = panel border title, content = current value) laid out like a table – rows of cells with `col_span`/`row_span` and `sc_form_add_skip` placeholders for spanned cells (same model as the table's `sc_cell_skip`/`sc_row_skip`). Per-field width is `ScFieldWidthMode` `pct`/`fixed`/`auto`; because it is a **shared column grid** a column has one width, taken from the **last** single-column (`col_span == 1`) field anchored in it (spanning fields just sum their columns). `pct` columns use **cumulative rounding** in `solve_columns` (an accumulator over the percentages, each column width = the difference of the floored cumulative targets `term_w·Σpct/100`), so consecutive `pct` columns collectively fill exactly – five 20% columns span the full width with no gap instead of losing up to one column each to independent flooring; every column is clamped to `FORM_MIN_COL` (6). Row heights come from each row's `height` (content lines); a `row_span` field fills the spanned rows. The whole form is **one** `sc_prompt_run` session (it must not be nested), modally split: **navigation** (arrows move the active box in 2D by edge-based nearest-neighbour over the grid – a remembered goal column/row preserves the original column on vertical moves and row on horizontal moves so stepping past a wide `col_span`/tall `row_span` cell stays in lane; arrows **cycle** around the grid edges by default, `ScFormOpts.no_cycle` stops at them; Tab/Shift-Tab always cycle by insertion order; the active box gets an `accent` border. Tab/Shift-Tab also work **while editing** – they commit the current field and open the next, bool/multiline fields are only focused) and **editing** (Enter opens an editor in the region BELOW the grid – the box keeps showing the committed value; a second Enter validates+saves and re-renders, Esc discards). **`ScFormOpts.autoedit`** opens the initial field's editor immediately at start (skips navigation mode; no-op on a bool field, multiline fields are only focused) – e.g. type a new record's title right away; implemented as a `begin_edit(self)` call at the top of `sc_form_run`. Bool fields toggle in place with Space/Enter (no editor). **select/multiselect** open a scrollable choice list in the below-region (↑/↓ or `j`/`k`; Space toggles a multiselect checkbox; Enter commits, Esc discards – the box keeps showing the committed value); a single-select snapshots its index, a multiselect a working `bool[]` copy restored on Esc. **Per-choice styles** (`sc_form_set_choice_styles(form, field, styles, n)`, stored as `Field.choice_styles`, copied; `n` may be < the choice count): color individual choices – applied to the **dropdown rows** (the cursor row unions the choice attrs with bold and forces the accent fg so it stays clearly marked; other rows get the full choice style) **and** to the **single-select value in the grid cell** (`capture_field` picks `choice_styles[sel]`). **Value-dependent cell style** (`ScFieldOpts.value_style`, an `ScFieldCellStyle` callback `(form, field, ctx) → ScTextStyle` + `value_style_ctx`): styles only the cell's value text, **called on every render** so the color updates live (e.g. color a date red/green by overdue/today while the picker is open – query the live value via `sc_form_get_*`); returns zero-init for "default", and `value_style` overrides the per-choice style when both are set. Disabled (read_only/not_selectable) and placeholder cells stay dim regardless. **date** opens a month grid below the grid (arrows day/week, PageUp/Down or `<`/`>` month, Shift+PageUp/Down year, Enter picks; Monday week start; self-contained date math mirroring `datepicker.c`); the box shows `YYYY-MM-DD`. With `ScFieldOpts.date_optional` the date may be absent (a zeroed `initial` starts the field empty showing `–`, Delete/Backspace clears it while editing, and `sc_form_get_date` returns `false`); a `Field.date_set` flag tracks presence. **multiline text** (`ScFieldOpts.multiline`): the value may hold newlines (shown across the box's content lines, split on `\n`) and is edited **via the external editor** – **Enter** or `ScFormOpts.editor_key` (zero-init = Ctrl-G) opens `ScFormOpts.editor` (NULL = `$VISUAL`/`$EDITOR`/nvim/vi) on the active field, reusing the engine's `ScPromptEditor` + `edit_get`/`edit_set` hooks. **`ScFormOpts.editor_suffix`** (borrowed, copied; NULL/empty = none) is passed through `ScPromptEditor.suffix` to `sc_run_editor`, which builds the temp file as `sparcli-edit-XXXXXX<suffix>` via `mkstemps` (e.g. `.md` so the editor detects the filetype; plain `mkstemp` when there is no suffix). text_input/textarea pass `suffix = NULL`. Enter is wired through the new optional `ScPromptVTable.wants_editor(state,key)` predicate (the engine runs the editor action when it returns true, in addition to the configured chord); `form_edit_get` returns NULL for non-multiline fields so the editor key is a no-op there (the engine's `run_editor_action` now treats a NULL `edit_get` as a no-op). The below-grid **editor region is framed in an accent panel at the full grid width** (same color as the active cell, label as title), preceded by a separator **rule** and drawn on a gray background that covers both the content **and the frame/border** (`ScFormOpts.edit_bg`, zero-init = `#2b2b2b`, customizable), with the key hint stacked dim beneath it; nav mode stays an unframed dim help/`form_err` line. Field widths support `col_span`/`row_span` so e.g. a select can span two columns. `Ctrl-D` submits but first enforces `required` (empty text/number or zero-checked multiselect blocks submit, jumps to the field, shows a red `form_err`); `Esc` in navigation cancels (`vtable.capture_escape` routes Esc per mode). Text/number editors reuse `ScLineEditor`; number commit parses with `strtod`; optional per-field `validate` keeps the editor open with a red error line. **`ScFieldOpts.read_only`** renders a field but blocks every mutation (`begin_edit` returns early so no editor opens, a bool does not toggle, value cycling is skipped); **`ScFieldOpts.not_selectable`** removes a field from all focus navigation (`neighbor`/`next_selectable`/`first_selectable` skip it, the initial active field and `autoedit` land on the first selectable one, `form_focus` redirects, and `field_required_unmet` treats it as satisfied so it never blocks submit) – the two combine for a display-only, unfocusable summary field (edited elsewhere, e.g. via a shortcut). Both render **dimmed** (no accent border even if active) and are zero-init = off; a form whose every field is `not_selectable` does not hang (the cycling loops are bounded, `first_selectable` falls back to 0). **Custom shortcuts while editing:** the form sets the engine's `suppress_char_shortcuts` predicate (`form_suppress_char_shortcuts` = `editing || ed_active`, mirroring the fuzzy finder's insert mode), so a bare printable-letter shortcut chord (mods==0, `SC_KEY_CHAR`) reaches the open inline editor as text instead of firing; in navigation mode it fires normally. Modifier chords (Ctrl/Alt) and non-character keys are unaffected. **`ScFormOpts.fullscreen`** (+ `header` borrowed `ScRendered`, `valign`, `valign_scope`): like the fuzzy fullscreen mode – `form_render` wraps the mosaic with `sc_fullscreen_compose(body, header, footer, valign, scope, bottom_reserve)` to pin the header and align the block within the terminal; run inside an `sc_altscreen_begin()` session for a consistent shell alongside a fullscreen finder. The grid is fixed-size by default (leftover space → `valign` pad), but a field with **`ScFieldOpts.fill_height`** grows its row to fill the remaining terminal height: `form_render` runs in two passes (body via `form_build_body(…, with_edit)`, then the fill row is extended by `term_h − footer − header − frame_lines` and the body rebuilt). `height` is the minimum, the first fill field wins, and it is a no-op outside fullscreen or when `free_rows ≤ 0`. **`ScFormOpts.valign_scope`** (`ScVAlignScope`, in `sparcli_core.h`) selects what `valign` moves: `SC_VALIGN_SCOPE_ALL` (zero-init) aligns the whole header+grid+footer block as one unit (the fill-pass path); `SC_VALIGN_SCOPE_CONTENT` builds the content (title+grid) and footer (edit region + hint) **separately** via `form_build_body(…, with_edit=false)` and pins them top/bottom, aligning only the grid in the gap (`[header][pad][grid][pad][footer]`, total height = `term_h − bottom_reserve`; no fill-pass). `sc_fullscreen_compose` now takes the borrowed `footer` + `scope`; its `blank_lines(n)` helper emits **exactly** `n` blank rows (a string of `n` newlines – an off-by-one was fixed so MIDDLE/BOTTOM pads and the CONTENT layout reach the bottom row precisely; previously each pad was one short). Rendering: each field is captured as a fixed-size panel (`sc_capture_panel_text`), then a **2D line-buffer compositor** (`compose_grid`) blits every panel into the mosaic at its `(x,y)` – robust to arbitrary spans where nested `sc_columns`/`sc_vstack` would not suffice. **Dirty tracking:** each field snapshots its initial value at add time; `sc_form_modified()` reports whether any field now differs (per-type compare in `field_modified`; useful for an "unsaved changes?" prompt on Esc), and **`ScFormOpts.modified_marker`** (e.g. `"[*] "`, copied) prefixes a changed field's box title – appearing/disappearing live as the value diverges from / returns to the snapshot. Getters: `sc_form_get_{string,number,bool,choice,checked,date}`. Frame builders `sc_form_frame` / `sc_form_frame_edit(form,field)` (input_internal.h) for snapshots (the latter opens the editor). The choice-list + month-grid logic are **self-contained** in `form.c` (NOT extracted from `select.c`/`datepicker.c`, to avoid destabilizing their golden tests). Theme: `sc_theme_apply_form` (called in `sc_form_new`) inherits accent/title(=prompt)/summary/hint styles + hint layout/pos from the global `ScInputTheme`. Tests: `tests/input/logic/test_form.c`, style snapshots in `test_style_form.c`, PTY cases `form-edit-text`/`-bool-toggle`/`-edit-cancel`/`-esc-cancels`/`-number-edit`/`-select`/`-multiselect`/`-select-esc-keeps`/`-required-blocks`/`-date`/`-tab-edit-advance`/`-tab-bool-focus`/`-nav-robust`/`-nav-right-spans`/`-nav-down-spans`/`-nav-goal-col`/`-nav-cycle`/`-nav-no-cycle`/`-date-clear`/`-date-optional-empty`/`-autoedit`/`-read-only`/`-read-only-bool`/`-not-selectable`/`-all-not-selectable`/`-shortcut-suppressed-editing`/`-shortcut-nav-fires`. Example `examples/c/apps/form_demo.c`. **Phases A–D done** (text/number/bool/select/multiselect/date + theme). **Bindings:** C++ `sparcli::Form` (`FieldOpts`/`FormOpts` aliases; `std::tm` dates), Rust `sparcli::Form` (`FieldOpts`/`FormOpts`/`FieldWidthMode`; `Date` dates), Python `sc.Form` (`FieldOpts`/`FormOpts` dataclasses; `datetime.date`). `fill_height`, `editor_suffix` and `valign_scope` (+ the `ScVAlignScope`/`ValignScope` enum) are wrapped in all three – C++ gets them free via the struct aliases, Rust/Python add the fields explicitly (Rust hand-edited `sparcli-sys/src/bindings.rs`: `fill_height`@20 in `ScFieldOpts`, `editor_suffix`@152 + `valign_scope`@176 in `ScFormOpts`, size 80/200; Python via the cdef + dataclass). `read_only`@72 + `not_selectable`@73 are likewise appended to `ScFieldOpts` (size 72→80) and wrapped in all three bindings. **Per-choice styles + value-dependent cell style** (`sc_form_set_choice_styles` + `ScFieldOpts.value_style`/`value_style_ctx`, appended @80/@88, `ScFieldOpts` size 80→96): **C + C++ only** – C++ `Form::set_choice_styles(field, vector<TextStyle>)` and `value_style` via the `FieldOpts` alias (a raw fn ptr, like the C++-only `validate`). Rust/Python only mirror the new `ScFieldOpts` layout (so the ABI stays correct); they do **not** expose these, consistent with the C++-only `validate` callback. Tests: `tests/input/logic/test_form.c` (per-choice cell color + `value_style` cell color, raw-ANSI assertions), C++ `test_cpp.cpp` (`form++: set_choice_styles + value_style build`). The per-field `validate` callback is exposed in C++ only (raw fn ptr); Rust/Python omit it. The `sparcli form` CLI subcommand is deliberately **not** provided (a 2D grid form has no clean command-line shape).
- **Key-hint footer:** every widget shows a dim footer (e.g. `↑/↓ move · enter select · esc cancel`) by default; override the text with `hint`, restyle with `hint_style`, and choose its layout with `hint_layout` (`ScHintLayout`: `SC_HINT_INLINE` one `·`-separated line / `SC_HINT_STACKED` one hint per line / `SC_HINT_HIDDEN` none; zero-init `SC_HINT_LAYOUT_DEFAULT` inherits the theme, then inline). Stacked is rendered by splitting the hint on ` · `. Its placement is `hint_pos` (`ScHintPosition`: `SC_HINT_POS_TOP`/`_BOTTOM`(default)/ `_LEFT`/`_RIGHT`; left/right sit beside the widget, top-aligned), orthogonal to layout. `sc_compose_hint` (input_internal.h) builds the hint block and places it: vstack for top/bottom, a 2-column `ScColumns` for left/right. **Frame alignment:** for a boxed widget the footer is indented to the box content column so it lines up under the framed content – `sc_compose_hint`'s `indent` arg (= `sc_box_content_left(box)` = `margin.left + border + padding.left`) shifts the widget's own hint (top/bottom only), and the engine indents the labeled-shortcut footer the same amount via the `ScPromptVTable.hint_indent(state)` accessor (`sc_indent_rendered`). Inline (unboxed) widgets keep the footer at column 0.
...
- **Custom shortcuts** (`input/sparcli_shortcut.h`): every widget's `Sc*Opts` carries `shortcuts` / `n_shortcuts` (borrowed `ScShortcut[]`) and an optional `out_shortcut_id`. The **prompt engine** (`prompt.c`) matches them before `on_key` – so one implementation covers all widgets – after the reserved cancel keys (Esc / Ctrl-C can't be bound). Two opt-in `ScPromptVTable` hooks let a widget bend this: `capture_escape` forwards `Esc` to `on_key` instead of cancelling (so it can be repurposed, e.g. the modal fuzzy's insert→normal switch; `Ctrl-C`/EOF still cancel), and `suppress_char_shortcuts(state)` skips bare-character shortcut chords **and the editing keys (Backspace/Delete)** while it returns true (the modal fuzzy's insert mode, so those keys edit the query instead of firing a shortcut bound to them). Both are zero-init/NULL for every other widget = today's behavior. Build chords with `sc_key_ctrl('e')` / `sc_key_fn(2)` / `sc_key_alt('e')` / `sc_key_special(SC_KEY_LEFT)` / `sc_key_mod(SC_KEY_UP, SC_MOD_ALT)` (named key + Shift/Alt/Ctrl, incl. combos). **Char chords are case-sensitive** (`p` ≠ `P`; `P` arrives as the codepoint `'P'` = Shift+p) – only **Ctrl** folds case (the terminal sends the same byte for Ctrl+p/Ctrl+P), so `sc_key_ctrl('e')`/`('E')` still match. Everything distinguishable is bindable **except** `Ctrl-C`/`Esc` (reserved = cancel) and `Ctrl-H/I/J/M` (delivered as Backspace/Tab/Enter). `SC_SHORTCUT_RETURN` ends the prompt and writes the fired `id` to `*out_shortcut_id` (`-1` = normal submit); the widget still returns its value. `SC_SHORTCUT_CALLBACK` runs `on_fire(id, user)` in place and stays open unless it returns `false`; `sc_select_cursor` / `sc_fuzzy_cursor_index` expose the live selection to such callbacks, and `sc_select_remove` / `sc_fuzzy_remove` delete the highlighted item live (the emptied-list run tails are guarded). A callback can't open a second prompt (single-session), so the "edit an item" pattern is a RETURN shortcut + `sc_select_label` / `sc_select_set_label` + `sc_select_set_cursor` in a caller re-run loop (see `examples/c/input/shortcuts_theme.c`). A shortcut with a `hint_label` is shown in a dim footer the **engine** stacks under every frame (key name via `sc_key_chord_name`, e.g. `^X delete`), discoverable on any widget with no per-widget code. `sc_key_chord_name` renders F-keys (`F2`), arrow/enter/tab **glyphs** (`↑`/`↵`/`⇥`), word names for the other named keys (`Del`/`Bksp`/`Home`/`End`/`PgUp`/`PgDn`/`Esc`) and the bare letter for `SC_KEY_CHAR`, each with a modifier prefix (`^`=Ctrl, `M-`=Alt, `S-`=Shift) – so Alt+Up is `M-↑` and Ctrl-X is `^X`. The key decoder now also yields `SC_KEY_F1..F12`, Alt via an `ESC`-prefix (`ScKey.mods & SC_MOD_ALT`), generic Ctrl-letters as `SC_KEY_CHAR` + `SC_MOD_CTRL`, and **modified named keys** (Shift/Alt/Ctrl + arrows/Home/End via the `ESC[1;<mod>X` parameter → `SC_MOD_SHIFT`/`_ALT`/`_CTRL` on the key's type). The C++ wrapper adds `sparcli::Shortcuts` (a builder with a `std::function` callback arena) + `key_ctrl/key_fn/key_alt/key_char/key_special/key_mod/key_shift`; `sc.apply(opts)` wires it and `sc.fired()` reads the result. Rust/Python expose the same builders plus the named-key shorthands (`key_delete/key_backspace/key_home/key_end/key_pageup/key_pagedown/key_backtab/key_esc`) and chainable modifier adders on the chord (`key_up().shift()/.alt()/.ctrl()`) – equivalent to `sc_key_mod`.
- **Shortcut display metadata + auto-built help screen** (`src/input/shortcut_help.c`): `ScShortcut` carries three display fields beyond `hint_label` – `help_text` (longer description for the help screen; `NULL` → `hint_label`), `section` (help-screen group title; `NULL` → `"Other"`), and `hide_in_footer` (keep the binding but drop it from the footer; `build_shortcut_hint` + `sc_shortcut_hint_rows` skip it). All zero-init, so existing initializers are unaffected. `sc_shortcut_help_show(rows, n, opts)` renders a modal, filterable, **full-terminal-height** help screen – it is a thin config of the fuzzy finder in table mode (`fullscreen`+`valign=TOP`+`box.width_mode=SC_WIDTH_FULL`+`stretch_columns` on the description column; key column padded to a global max so the layout is scroll-stable). Rows are `ScShortcutHelpRow { section, key_display, desc }`: a `section` header **or** a key/description line, so **help-only rows** (built-in widget keys like `↑/↓`, not bound `ScShortcut`s) and bindings interleave in display order. `sc_shortcut_help_show_from(items, n, opts)` is the convenience that derives rows from a bound set (key via `sc_key_chord_name`, desc `help_text`→`hint_label`, grouped by `section`, insertion order; no-description shortcuts skipped). `ScShortcutHelpOpts { title, accent(0=yellow), footer_hint, in_alt_screen }`: fullscreen needs an alternate screen, so `_show` opens its own (`sc_altscreen_begin`/`_end`) **unless** `in_alt_screen` (caller – e.g. a long-running TUI – already holds one, so no nesting). Test hooks `sc_shortcut_help_frame`/`_from` (input_internal.h). Tests: `tests/input/logic/test_shortcut_help.c` (footer visibility, grouping, fallback, help-only), style snapshot `test_style_help.c`, PTY `shortcut-help-esc`/`-filter`/`-in-alt`. **Bindings (full parity):** C++ `ShortcutDisplay`{footer,help,in_footer} + rich `on_return`/`on_callback` overloads (thin label wrappers kept) + `section`/`help_row` builder methods + `show_shortcuts(sc, ShortcutHelpOpts)`; Rust `ShortcutDisplay`/`on_return_d`/`on_callback_d`/`section`/`help_row` + `show_shortcuts(&sc, ShortcutHelpOpts)` (sys struct hand-edited: `ScShortcut` 48→72, new `ScShortcutHelpRow`/`ScShortcutHelpOpts`); Python `Shortcuts.on_return/on_callback` gain `help=`/`in_footer=` kwargs + `section()`/`help_row()` + `sc.show_shortcuts(sc, sc.ShortcutHelpOpts(...))`. Examples `examples/c/input/shortcuts_help.c` + `examples/cpp/input/shortcuts_help.cpp`.
...
| `sc_utf8_decode(cursor, end, &cp)` | Bounds-safe UTF-8 codepoint decode; returns bytes consumed (1–4), always advances |
| `sc_codepoint_width(cp)` | Display width of a codepoint: 0 (combining/zero-width), 2 (East-Asian Wide/Fullwidth/emoji), else 1. Ambiguous symbols (below U+2E80: box drawing, arrows, `✔`/`✖`, …) stay 1 |
| `sc_utf8_string_length(string, byte_length)` | Visible column count of a UTF-8 byte sequence (**display-width-aware** via `sc_codepoint_width`; ANSI/control = 0 cols) |
| `sc_utf8_trim_to_cols(string, max_columns)` | Byte count fitting within `max_columns` display columns; **never splits a double-width glyph** (may stop one column short – callers pad) |
...
- Word-wrap in tables (`ScColOpts.wrap = 1`) breaks on spaces only. If no space fits in the column width, the line is truncated. **Truncated (non-wrapped) cells end with an ellipsis (`…`)** – `render_cell_line_truncated` reserves one column for the glyph and trims the content to fit (`remaining - 1` columns + `…`); a column only 1 cell wide shows just `…`. The ellipsis inherits the cell's style/bg. Because `sc_utf8_trim_to_cols` never splits a **double-width glyph** (CJK/emoji), the clip can land one column short of the budget; the path measures what was actually kept and **pads the cell to the full column width** with `print_spaces_with_bg`, so a wide-glyph boundary never misaligns the border.
...
- `sc_terminal_size(int *w, int *h)` / `sc_term_width()` / `sc_term_height()`: current terminal size in cells (80x24 fallback); public wrappers over the internal `sc_terminal_width/height`. Bindings: C++ `sparcli::terminal_size()`, Rust `sparcli::terminal_size()`, Python `sc.terminal_size()`.
...
```c
ScLive *live = sc_live_begin((ScLiveOpts){ 0 });  /* .alt_screen, .show_cursor, .transient, .always, .prompt_rows, .valign, .valign_fixed_header */
...
| `valign` / `valign_fixed_header` | Vertical alignment on the **alt screen** (`SC_VALIGN_TOP` default / `MIDDLE` / `BOTTOM`): `draw_frame` pads with leading blank rows (whole frame+reserve block) or, with `valign_fixed_header`, a gap between the pinned frame and the reserve. The pads are part of the drawn region (counted in `prev_lines`), so the rewind + park stay consistent. Ignored off alt-screen. |
...
**Alt-screen session (`sc_altscreen_begin()`/`sc_altscreen_end()`):** a thin standalone alternate-screen scope (enter `\033[?1049h\033[H` + hide cursor / leave + show), reusing live's `cleanup_arm` crash/atexit restore; no-op off a terminal; one at a time. It does **not** draw – it just owns the screen for an app loop that runs **fullscreen widgets** (`ScFuzzyOpts.fullscreen` / `ScFormOpts.fullscreen`): open it once, run the widgets (each composes its own header + valign + body into the active screen via `sc_fullscreen_compose`, so switching them never flickers), then end it. Preferred over `sc_live` `prompt_rows` for a growing finder, since the widget – not a fixed reserve – owns its height. Bindings: C++ `sparcli::AltScreen` (RAII), Rust `AltScreen` (Drop), Python `sc.altscreen()` (context manager).

Tests: `tests/output/test_live.c` (`make test-output`; non-TTY path is golden-tested, escape emission + `prompt_rows` park/rewind verified via memstream + `always`); animated dashboard demo flagged `--no-animated`-skippable. Examples: `examples/c/output/live.c`, `examples/c/apps/repl_dashboard.c` (live + prompt + shortcut bar), `examples/c/apps/fullscreen_app.c` (alt-screen session + fullscreen fuzzy↔form, pinned rich header + valign, auto-grow).
...
| Dispatch | Each command node carries a borrowed `void *user_data` (`sc_args_cmd_set_userdata`/`_userdata`, never freed) so the matched node *is* the dispatch table - no name-compare switch. The C++ wrapper builds on it: `ArgsCmd::handler(std::function<int(const Args&)>)` stores a callback in an `Args`-owned arena (each on its own heap node → survives a move of `Args`), `Args::dispatch(matched)` fires it. `userdata()`/`userdata<T>()` is the raw passthrough sharing the same slot; `has_handler`/`dispatch` resolve against the arena so a raw pointer is never mis-cast as a handler |
...
- **File I/O**: every format has `sc_<fmt>_parse_file(path,...,err)` / `sc_<fmt>_write_file(value/doc,path,...)` convenience wrappers (shared `sc_serde_read_file`/`_write_file` in `file.c`); parse_file fills `err` (incl. "cannot open file") on read OR parse failure, write_file returns false on I/O error.
- **Shared infra**: `ScSerdeBuf` (`buf.c`, growable sticky-fail buffer, `sc_serde_append_utf8`) + `file.c` (read/write helpers) back every writer/string decoder.
- **C++** (`sparcli::serde`, `sparcli_serde.hpp`): RAII `Value` (+ `clone`/`remove`/`remove_at`/`merge`/`path`) + non-owning `View` (`std::optional` accessors, `path`); `json`/`toml`/`yaml` namespaces (`parse`/`parse_file` throw `ParseError`, `write`/`write_file`); `Csv`, `Markdown` + `Section` handle (both with `parse_file`/`write_file`). C++20.
