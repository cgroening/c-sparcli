...
- **Fuzzy** ranks by `sc_fuzzy_match` on each keystroke; matched characters are highlighted (bold+underline, accent fg) in the list and in the matched table cells (`render_table` builds those cells as highlighted `ScText` via `append_highlighted`, borrowed by the table and freed after capture); the query field scrolls horizontally (`‹`/`›`) when long; table view builds an `ScTableData` of the visible rows each frame (cursor row via row-bg) and `sc_vstack`s query + body + scroll-indicator + footer. `refilter` matches each row via `row_matches` across the columns selected by `opts.search_columns` (bitmask; `0` = all, the default), ranking by the best-scoring column – so a table query can hit (and highlight) any column, not just the first. The cursor cycles around the result-list ends by default; `opts.no_cycle` disables it. **Section navigation:** when sections are present (`sc_fuzzy_add_section`), **Tab jumps to the next section** (its first selectable row) and **Shift-Tab to the previous section's first row**, both cycling around the ends (`section_jump` over the `matches[]` display list); without sections Tab/Shift-Tab fall back to cursor down/up. Arrows / `j`/`k` / Ctrl-N/P stay single-step. **`opts.max_height`** (rows): caps the finder height so the result list scrolls within it (the existing `top`+`effective_visible` viewport) instead of overflowing; `0` auto-fits the terminal height, or set it to a live dashboard's `prompt_rows` reserve. `effective_visible()` subtracts the query line, scroll indicator, box frame and hint footer. When the list scrolls, a **right-edge scrollbar** (track `░` / thumb `█`, `attach_scrollbar` appends a gap + bar column to the body, reserving 2 interior cols) is drawn **by default** alongside the `↑ x–y/total ↓` text line; `opts.no_scrollbar` opts out. **`sc_fuzzy_refresh()`**: re-applies the query after appending rows mid-run (the `if (matches) refilter()` path, which now grows the `matches` array to the new row count); bind it from a `SC_SHORTCUT_CALLBACK` "refresh" key to add items live without closing the finder. **`opts.stretch_columns`** (bitmask, table view): when the `box` resolves to a width wider than the natural table (`SC_WIDTH_FULL`/`FIXED`, or `CONTENT`+`max_width`), the table is re-rendered with `total_width = interior` and the selected columns absorb the slack (the rest get `ScColOpts.no_stretch`); `0` = content-sized as before. The list view already fills a bounded box, so this is table-only. **`opts.fullscreen`** (+ `opts.header` borrowed `ScRendered`, `opts.valign`): full-screen mode — the finder fills the terminal, its list **grows with the items** up to the available height (`sc_terminal_height() − header_lines − chrome`) and only then scrolls, and each frame `fuzzy_render` wraps the result with `sc_fullscreen_compose(frame, header, valign)` so the pinned header + the leftover-space alignment (below the finder for TOP, above the header for BOTTOM, both for MIDDLE) **re-compute live** as the list grows. The finder does **not** own the alternate screen — run it inside an `sc_altscreen_begin()`/`sc_altscreen_end()` session (so switching widgets don't flicker).
  ...
  - **Sections** (`sc_fuzzy_add_section`): non-selectable headers. `refilter` emits each group as `[header][its matches]` and the header **only when the group matched** (`opts.section_counts` appends `" (k)"`). In the table they render as a colspan row over all columns. `opts.empty_text` shows a line when nothing matches.
