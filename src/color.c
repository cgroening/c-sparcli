...
static const ColorLayer SC_COLOR_LAYER_FOREGROUND = {3, 38};
static const ColorLayer SC_COLOR_LAYER_BACKGROUND = {4, 48};
...
static void apply_fg(ScColor clr) {
    apply_color_escape(clr, SC_COLOR_LAYER_FOREGROUND);
}

/** Emits the ANSI background color escape code for `clr`. */
static void apply_bg(ScColor clr) {
    apply_color_escape(clr, SC_COLOR_LAYER_BACKGROUND);
}
 ...
 * - `-2` (`SC_COLOR_NONE`): Mo output, returns immediately.
 * - `-1` (24-bit RGB): Emits `\033[{rgb_prefix};2;r;g;bm`.
 * - `0` to `7` (named color): Emits `\033[{named}{index}m`.
 *
 * @param clr    Color to emit.
 * @param layer  Prefix constants - use `SC_COLOR_LAYER_FOREGROUND` or
 *               `SC_COLOR_LAYER_BACKGROUND`.
 */
static void apply_color_escape(ScColor clr, ColorLayer layer) {
