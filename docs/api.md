...
- **[Framework API (C)](api-framework.md)** – the application-framework helpers of the C library: XDG paths, pretty errors, logging, the subprocess helper (`sc_run`), layered config (`sc_config_*`), and the argument parser (subcommands, typed options, auto --help, zsh/bash/fish completion).
- **[Serde / data parsers (C + C++)](api-serde.md)** – the opt-in `serde/` layer: the shared `ScValue` model and JSON/CSV/TOML/YAML/Markdown readers and writers, plus the `sparcli::serde` C++ wrapper, and the **view** layer that renders those models to the terminal (`sc_value_render` / `sc_markdown_render`). Not part of the `sparcli.h` umbrella.
