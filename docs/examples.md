...
| `c/apps/fullscreen_app` | An alternate-screen shell: a title/subtitle panel header pinned above a vertically-centered (`ScLiveOpts.valign`) region that hosts a fuzzy finder and a form, switching between them. The finder caps its height (`ScFuzzyOpts.max_height`) so it scrolls, and a `^R` refresh key adds an item live via `sc_fuzzy_refresh`. |
